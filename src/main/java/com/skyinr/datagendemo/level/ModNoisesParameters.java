...
public class ModNoisesParameters {
    public static final Holder<NormalNoise.NoiseParameters> NOISE_PARAMETERS_DEMO_HOLDER;
    public static final ResourceKey<NormalNoise.NoiseParameters> NOISE_PARAMETERS_DEMO_KEY = ResourceKey.create(Registry.NOISE_REGISTRY, DataGenDemo.modLoc("noise_parameters_demo"));
    public static final NormalNoise.NoiseParameters NOISE_PARAMETERS_DEMO = new NormalNoise.NoiseParameters(
            -4,
            new DoubleArrayList(new double[]{1.0D,0.5D,0.23D,4.32D})
    );
    public static final WritableRegistry<NormalNoise.NoiseParameters> NOISE_PARAMETERS_REGISTRY = ModWorldgenProvider.registrable.ownedWritableRegistryOrThrow(Registry.NOISE_REGISTRY);

    static {
        NOISE_PARAMETERS_DEMO_HOLDER = NOISE_PARAMETERS_REGISTRY.register(NOISE_PARAMETERS_DEMO_KEY, NOISE_PARAMETERS_DEMO, Lifecycle.stable());
        //NoiseData Copy
    ...
    private static void register(ResourceKey<NormalNoise.NoiseParameters> p_194751_, int p_194752_, double p_194753_, double... p_194754_) {
        NOISE_PARAMETERS_REGISTRY.register(p_194751_, new NormalNoise.NoiseParameters(p_194752_, p_194753_, p_194754_), Lifecycle.stable());
    }
    //NoiseData Copy end
}
